...
/**
 * (Delegated) Implementation of hook_civicrm_tabs
 * @param $files array(string)
 */
function _volunteer_civix_civicrm_tabs( &$tabs, $entityID, $entity) {
  if ($entity == 'civicrm_event') {
...
function _volunteer_civix_civicrm_managed(&$entities) {
  ...
  foreach ($mgdFiles as $file) {
    ...
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'org.civicrm.volunteer';
      }
      $entities[] = $e;
    }
  }
 ...
 */
function _volunteer_civix_insert_navigation_menu(&$menu, $path, $item, $parentId = NULL) {
  static $navId;
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _volunteer_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item, $key);
      }
    }
    return $found;
  }
