CRM.$(function($) {
  ...
  //create a list of name prefixes
  var namePrefixes = ['additionalVolunteersTemplate'];
  var i = 1;
  while(i < qty) {
    namePrefixes.push("additionalVolunteers_" + i);
    i++;
  }
  for(var field in CRM.additionalVolunteers.fields) {
    for(var i in namePrefixes) {
      //Set the placeholder text on the profiles that were generated by Civi
      $("[name='" + namePrefixes[i] + "[" + field + "]']").attr("placeholder", CRM.additionalVolunteers.fields[field].title);
    }
  }
